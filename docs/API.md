...
- Every engine SHOULD expose `GET /api/version` returning `{"api_version": "1", "engine": "<python|django|php>", "knitweb": "<engine version>"}`. A client detects drift by comparing `api_version`. _(Endpoint to be added in this sprint; `1` is the current contract.)_
